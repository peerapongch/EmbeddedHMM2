...
update_rw_param <- function(param_current,param_formed_current,param_lprob_current,X_current,Y,rw_scale,dim){
  ...
  param_new <- param_current
  param_new[1:2] <- param_new[1:2] + rw_scale[1:2]*rnorm(2)
...
lambdaModel2_param <- function(ssm,N,L,N.mcmc.param=20,init=NULL,seed=NULL,rw_scale=c(0.01,0.01,0.01),sc_scale=0.01,rho.init=NULL,phi.init=NULL,delta.init=NULL,checkpoint.name=NULL){
  ...
  return(list(X_sample=X_sample[-1,,],param_sample=param_sample,N=N,L=L,init=init,
    seed=seed,X_pool=X_pool,acceptance_rate=acceptance_rate,rw_acceptance_rate=rw_acceptance_rate,sc_acceptance_rate=sc_acceptance_rate))
}