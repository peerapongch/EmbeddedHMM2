...
update_sc_param <- function(param_current,param_formed_current,param_lprob_current,X_current,Y,sc_scale,dim,T){
  # propose
  param_new <- param_current
  z <- rnorm(1)*sc_scale
  param_new[3] <- param_new[3] + z
  ...
  if(check_domain_delta){
    ...
    X_new <- X_current*param_formed_current$delta[1]/param_formed_new$delta[1]
...
update_rw_param <- function(param_current,param_formed_current,param_lprob_current,X_current,Y,rw_scale,dim){
  ...
  if(check_domain_phi && check_domain_rho && check_domain_delta){
    ...
    param_formed_new <- list(F=F,sigma_init=sigma_init,sigma=sigma,sigma_init_inv=sigma_init_inv,sigma_inv=sigma_inv,sigma_init_U = sigma_init_U, sigma_init_L = sigma_init_L, sigma_U = sigma_U, sigma_L = sigma_L, delta=delta_new)
...
update_rcr_param <- function(param_current,param_formed_current,param_lprob_current,X_current,Y,rcr_scale,dim){
  # propose
  param_new <- param_current
  param_new[1:2] <- param_new[1:2] + rcr_scale*rnorm(2)
  ...
  if(check_domain_phi && check_domain_rho){
    ...
    # make new sequence 
    # convert LU to UL decomposition
    P <- revdiag(rep(1,dim))
    U1 <- P %*% param_formed_current$sigma_L %*% P
    U2 <- P %*% sigma_L %*% P
    U3 <- P %*% param_formed_current$sigma_init_L %*% P
    U4 <- P %*% sigma_init_L %*% P 
    ...
    I <- diag(1,dim)
    # print((I - pre_L) %*% F)
    X_new <- matrix(logical(0),nrow=T,ncol=dim)
    X_new[1,] <- pre_1 %*% X_current[1,]
    for(t in 2:T){
      X_new[t,] <- (F - pre_L %*% param_formed_current$F) %*% X_current[t-1,] + pre_L %*% X_current[t,]
    }
    # compute prob
    param_lprob_new <- param_lprob(param_formed_new,X_new,Y,T)
    log_jacobian_adjust <- log(abs(prod(diag(pre_1))))+(T-1)*log(abs(prod(diag(pre_L))))
    log_acceptance_prob <- param_lprob_new-param_lprob_current+log_jacobian_adjust
...
lambdaModel2_param <- function(ssm,N,L,N.mcmc.param=20,init=NULL,seed=NULL,rw_scale=c(0.01,0.01,0.01),rcr_scale=c(0.01,0.01),sc_scale=0.01,rho.init=NULL,phi.init=NULL,delta.init=NULL,checkpoint.name=NULL){
  ...
  sc_acceptance_rate <- 0
  rcr_acceptance_rate <- 0
  ...
  for(i in seq(2,2*N,2)){
    ...
    for(j in seq(1,N.mcmc.param,3)){
      ...
      # rcr 
      param_update_out <- update_rcr_param(param_current,param_formed_current,param_lprob_current,X_current,Y,rcr_scale,dim)
      if(any(param_current != param_update_out$param_new)){
        rcr_acceptance_rate <- rcr_acceptance_rate + 1
      }
      param_formed_current <- param_update_out$param_formed_new
      param_current <- param_update_out$param_new
      ...
      # then shsc update 
      param_update_out <- update_sc_param(param_current,param_formed_current,param_lprob_current,X_current,Y,sc_scale,dim,T)
      if(any(param_current != param_update_out$param_new)){
        sc_acceptance_rate <- sc_acceptance_rate + 1
      }
      param_formed_current <- param_update_out$param_formed_new
      param_current <- param_update_out$param_new
      X_current <- param_update_out$X_new
      param_lprob_current <- param_update_out$param_lprob_new
      param_sample[start_index+j+2,] <- param_current
    }
    ...
    if(i %% 100 == 0){
      ...
      print(rw_acceptance_rate/(N.mcmc.param/3*2*i))
      print(rcr_acceptance_rate/(N.mcmc.param/3*2*i))
      print(sc_acceptance_rate/(N.mcmc.param/3*2*i))
      print("----")
    }
  }

  rw_acceptance_rate <- rw_acceptance_rate/(N.mcmc.param/3*2*N)
  sc_acceptance_rate <- sc_acceptance_rate/(N.mcmc.param/3*2*N)
  rcr_acceptance_rate <- rcr_acceptance_rate/(N.mcmc.param/3*2*N)

  
  return(list(X_sample=X_sample[-1,,],param_sample=param_sample,N=N,L=L,init=init,
    seed=seed,X_pool=X_pool,acceptance_rate=acceptance_rate,rw_acceptance_rate=rw_acceptance_rate,sc_acceptance_rate=sc_acceptance_rate,rcr_acceptance_rate=rcr_acceptance_rate))
}