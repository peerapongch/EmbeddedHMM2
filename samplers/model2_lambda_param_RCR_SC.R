...
lambdaModel2_param <- function(ssm,N,L,N.mcmc.param=20,init=NULL,seed=NULL,rcr_scale=c(0.01,0.01),sc_scale=0.01,rho.init=NULL,phi.init=NULL,delta.init=NULL,checkpoint.name=NULL){
  ...
  for(i in seq(2,2*N,2)){
    ...
    if(i %% 100 == 0){
      ...
      print(rcr_acceptance_rate/(N.mcmc.param*i))
      print(sc_acceptance_rate/(N.mcmc.param*i))
      print("----")
    }
  }

  # rw_acceptance_rate <- rw_acceptance_rate/(N.mcmc.param/3*2*N)
  sc_acceptance_rate <- sc_acceptance_rate/(N.mcmc.param*N)
  rcr_acceptance_rate <- rcr_acceptance_rate/(N.mcmc.param*N)

  
  return(list(X_sample=X_sample[-1,,],param_sample=param_sample,N=N,L=L,init=init,
    seed=seed,X_pool=X_pool,acceptance_rate=acceptance_rate, #rw_acceptance_rate=rw_acceptance_rate,
    sc_acceptance_rate=sc_acceptance_rate,rcr_acceptance_rate=rcr_acceptance_rate))
}