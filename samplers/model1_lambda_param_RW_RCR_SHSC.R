...
update_rcr_param <- function(param_current,param_formed_current,param_lprob_current,X_current,Y,rcr_scale,dim){
  ...
  if(check_domain_phi && check_domain_rho){
    ...
    for(t in 2:T){
      X_new[t,] <- (I - pre_L) %*% F %*% X_new[t-1,] + pre_L %*% X_current[t,]
    }
    # compute prob
    param_lprob_new <- param_lprob(param_formed_new,X_new,Y,T)
...
lambdaModel1_param <- function(ssm,N,L,init=NULL,seed=NULL,N.mcmc.param=20,rw_scale=c(0.01,0.01,0.01,0.01),rcr_scale=c(0.1,0.1),shsc_scale=c(0.01,0.1),rho.init=NULL,phi.init=NULL,delta.init=NULL,c.init=NULL,checkpoint.name=NULL){
  ...
  shsc_acceptance_rate <- shsc_acceptance_rate/(N.mcmc.param/3*2*N)
  rw_acceptance_rate <- rw_acceptance_rate/(N.mcmc.param/3*2*N)

  return(list(X_sample=X_sample[-1,,],param_sample=param_sample,N=N,L=L,init=init,seed=seed,X_pool=X_pool, acceptance_rate=acceptance_rate,rcr_acceptance_rate=rcr_acceptance_rate,shsc_acceptance_rate=shsc_acceptance_rate,rw_acceptance_rate=rw_acceptance_rate))
}
